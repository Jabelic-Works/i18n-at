...
メッセージが、それを所有するコンポーネントや route の近くにあると、別ディレクトリの翻訳ファイルを同期する手間が減り、レビュー対象の変更そのものに集中しやすくなります。
...
reviewer は UI と copy を同じ場所で確認できます。UI 変更だけ見て copy 変更を見落としたり、parameter mismatch を見逃したりしにくくなります。
...
- product team が UI と copy をまとめて持つ
...
- 生成差分を review しやすい
- UI と copy の drift が起きにくい

review が不要になるわけではありませんが、良い review を安くできます。
